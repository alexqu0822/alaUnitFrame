...
local function _get_aura_(unit, index, filter)
...
function UF.guess_tick_is_mana_regen(U, diff)
	...
	-- else
		...
		-- if alaUnitFrameSV.feedback_on and U.powerType == 0 then
		-- 	local feedback = alaUnitFrameSV.feedback[U.CLASS][U.LEVEL];
		-- 	if feedback then
		-- 		feedback[feedback.pos] = { spirit, mp5, tick, diff, (U.maxPowers[0] - U.curPowers[0]) <= 0.5, };
		-- 		feedback.pos = feedback.pos + 1;
		-- 		if feedback.pos > 1024 then
		-- 			feedback.pos = 1;
		-- 		end
		-- 	end
		-- end
	...
	function T:Update()
		if UnitExists(target) then
			...
			if UnitIsPlayer(target) then
				local _, ec = UnitClass(target);
				local color = ec and RAID_CLASS_COLORS[ec];
				if color then
...
function UF.hookUnitFrame(uf, unit, portraitPos, overrideTexture, ufBgCoord, hook_bar, create3DPortrait, createClass, createTarget, targetPos, createAura)
	...
	U:SetFrameLevel(uf:GetFrameLevel() + 1);
	U:EnableMouse(false);
	...
	if overrideTexture == false then
		...
		if tf and tf:GetObjectType() == "Texture" then
			...
			local BG = U:CreateTexture(nil, "BORDER");
	...
	if hook_bar and hb and pb then
		...
		if createBar then
			hBar = U:CreateTexture(nil, "BACKGROUND");
			hBar:SetTexture("Interface\\TargetingFrame\\UI-StatusBar");
			hBar:ClearAllPoints();
			hBar:SetPoint("TOPLEFT", hb:GetStatusBarTexture(), "TOPLEFT", 0, 0);
			hBar:SetPoint("BOTTOMRIGHT", hb:GetStatusBarTexture(), "BOTTOMRIGHT", 0, 0);
			hBar:SetVertexColor(1, 0, 0);
			pBar = U:CreateTexture(nil, "BACKGROUND");
			pBar:SetTexture("Interface\\TargetingFrame\\UI-StatusBar");
			pBar:ClearAllPoints();
			pBar:SetPoint("TOPLEFT", pb:GetStatusBarTexture(), "TOPLEFT", 0, 0);
			pBar:SetPoint("BOTTOMRIGHT", pb:GetStatusBarTexture(), "BOTTOMRIGHT", 0, 0);
			pBar:SetVertexColor(pb:GetStatusBarColor());
			-- hooksecurefunc(pb:GetStatusBarTexture(), "SetVertexColor", function(_, ...)
			-- 	pBar:SetVertexColor(...);
			-- end);
			hBar:Show();
			pBar:Show();
		...
		if createBar or createVal or createPer then
			function U:UpdateHealth()
				...
				if hmv > 0 then
					...
					hBar:SetVertexColor(0.0, 1.0, 0.0, 0.0);
					hVal:Hide();
					hPer:Hide();
				end
			end
	...
	if create3DPortrait and portrait2D then
		...
		portrait3D:SetFrameLevel(1);
		portrait3D:ClearAllPoints();
		portrait3D:SetPoint("CENTER", portrait2D, "CENTER", 0, -1);
		portrait3D.bg = portrait3D:CreateTexture(nil, "BACKGROUND");
	...
	if createClass then
		...
		if portraitPos == "LEFT" then
			class:SetPoint("TOPRIGHT", -120, -10);
		else
			class:SetPoint("TOPLEFT", 120, -10);
		end
		class:SetFrameLevel(U:GetFrameLevel() + 1);
		class.bg = class:CreateTexture(nil, "BACKGROUND");
		...
		class.border = class:CreateTexture(nil, "OVERLAY");
		...
		function U:UpdateClass()
			...
			if U.CLASS then
				-- alaUnitFrameSV.feedback[U.CLASS] = alaUnitFrameSV.feedback[U.CLASS] or {  };
			end
		end
		U:UpdateClass();
	...
	function U:UpdateLevel()
		...
		if U.CLASS then
			-- alaUnitFrameSV.feedback[U.CLASS][U.LEVEL] = alaUnitFrameSV.feedback[U.CLASS][U.LEVEL] or { pos = 1, };
		end
	end
...
function UF.coverTexture(uf, covered, layoutLevel, subLayoutLevel)
	...
	layoutLevel = layoutLevel or covered:GetDrawLayer() or "OVERLAY";
	local tex = uf:CreateTexture(nil, layoutLevel);
	tex:SetDrawLayer(layoutLevel, 7);	--[-8~7]
...
function UF.coverFontString(uf, covered, layoutLevel, subLayoutLevel)
	...
	layoutLevel = layoutLevel or covered:GetDrawLayer() or "OVERLAY";
	local fontString = uf:CreateFontString(nil, layoutLevel);
	fontString:SetDrawLayer(layoutLevel, 7);	--[-8~7]
...
local function init()
	do	--	player & pet
		...
		PU.LevelText = UF.coverFontString(PU, PlayerLevelText);
		PU.HitIndicator = UF.coverFontString(PU, PlayerHitIndicator);
		PU.PVPIcon = UF.coverTexture(PU, PlayerPVPIcon);
		PU.LeaderIcon = UF.coverTexture(PU, PlayerLeaderIcon);
		PU.MasterIcon = UF.coverTexture(PU, PlayerMasterIcon);
		PU.RestIcon = UF.coverTexture(PU, PlayerRestIcon);
		PU.AttackIcon = UF.coverTexture(PU, PlayerAttackIcon);
		PU.AttackBackground = UF.coverTexture(PU, PlayerAttackBackground);
		--PU.StatusTexture = UF.coverTexture(PU, PlayerStatusTexture);
		PU:UpdateLevel();
	...
	do	--	target & TOT
		...
		TU.LevelText = UF.coverFontString(TU, TargetFrameTextureFrameLevelText, "OVERLAY");
		TU.HighLevelTexture = UF.coverTexture(TU, TargetFrameTextureFrameHighLevelTexture);
		TU.PVPIcon = UF.coverTexture(TU, TargetFrameTextureFramePVPIcon);
		TU.LeaderIcon = UF.coverTexture(TU, TargetFrameTextureFrameLeaderIcon);
		TU.RaidTargetIcon = UF.coverTexture(TU, TargetFrameTextureFrameRaidTargetIcon);
		...
		local TOT = UF.hookUnitFrame(TargetFrameToT, 'targettarget', "LEFT", nil, { 0.015625, 0.7265625, 0.0, 0.703125, }, { true, true, false, 0, nil, nil, nil, }, nil, false, { 90, 1.0, });
		local update_timer = GetTime() + target_UPDATE_INTERVAL;
	...
	do	--	party
		...
		for i = 1, 4 do
			local U = UF.hookUnitFrame(_G["PartyMemberFrame" .. i], 'party' .. i, "LEFT", nil, nil, { true, true, true, nil, nil, false, 10, 1.0, }, true, false, { 50, 1.0, }, nil, true);
			U.GROUP_ROSTER_UPDATE = GROUP_ROSTER_UPDATE;
			UF.regEvent(U, "GROUP_ROSTER_UPDATE");
			U.PVPIcon = UF.coverTexture(U, _G["PartyMemberFrame" .. i .. "PVPIcon"]);
			U.LeaderIcon = UF.coverTexture(U, _G["PartyMemberFrame" .. i .. "LeaderIcon"]);
			U.MasterIcon = UF.coverTexture(U, _G["PartyMemberFrame" .. i .. "MasterIcon"]);
		end
	end
...
do
	...
	local function CheckButtonOnClick(self)
		...
		if key then
			...
			-- elseif key == "feedback_on" then
			-- 	alaUnitFrameSV[key] = on;
			-- 	wipe(alaUnitFrameSV.feedback);
			else
	...
	function UF.PLAYER_ENTERING_WORLD(self, event)
		...
		self:SetScript("OnEvent", UF.OnEvent);
...
UF:SetScript("OnEvent", UF.PLAYER_ENTERING_WORLD);
