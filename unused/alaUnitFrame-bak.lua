...
function UF.create_extra_power0(U, unit, portraitPos, perOfs)
	...
	if pb then
		...
		function extra_power0:UpdatePower()
			...
			if MT.GetConfig(configKey, "pVal") then
				epVal:SetText(pv .. " / " .. pmv);
				epVal:Show();
			else
				epVal:Hide();
			end
			if MT.GetConfig(configKey, "pPer") then
	...
	if hook_bar and hb and pb then
		...
		if createBar or createVal or createPer then
			function U:UpdateHealth()
				...
				-- local hv, hmv = UnitHealth(unit), UnitHealthMax(unit);
				if hmv > 0 then
					if MT.GetConfig(configKey, "hVal") then
						hVal:SetText(hv .. " / " .. hmv);
						hVal:Show();
					else
						hVal:Hide();
					end
					if MT.GetConfig(configKey, "hPer") then
						hPer:SetText(getTextPer(hv, hmv));
						hPer:Show();
					else
						hPer:Hide();
	...
	if createClass then
		...
		if unit == 'player' or unit == 'party1' or unit == 'party2' or unit == 'party3' or unit == 'party4' then
			function U:UpdateClass()
				...
				if MT.GetConfig(configKey, "class") and U.CLASS then
...
local function init()
	...
	--	setting
		...
		if (VT.IsCata or VT.IsWrath or VT.IsTBC) and alaUnitFrameSV.ShiftFocus then
		...
		for unit, U in next, UF.frames do
			...
			U:text_alpha(MT.GetConfig(configKey, "text_alpha"));
		end
...
do
	...
	local function sliderRefresh(self)
		if self.key == "text_alpha" then
			local value = MT.GetConfig(alaUnitFrameSV.which or 'general', self.key);
			self:SetValue(value);
	...
	function UF.ADDON_LOADED(self, event, addon)
		...
		if alaUnitFrameSV._ver == nil or alaUnitFrameSV._ver < 20241201.0 and VT.IsVanilla then
