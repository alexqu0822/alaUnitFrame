...
local isWLK = WOW_PROJECT_ID == WOW_PROJECT_WRATH_CLASSIC;				--	11
local isBCC = WOW_PROJECT_ID == WOW_PROJECT_BURNING_CRUSADE_CLASSIC;	--	5
local IsVanilla = WOW_PROJECT_ID == WOW_PROJECT_CLASSIC;				--	2
local isRetail = WOW_PROJECT_ID == WOW_PROJECT_MAINLINE;				--	1

if not isCata and not isWLK and not isBCC and not IsVanilla then
	return;
end
...
local defConfig = {
	...
	power_restoration = isBCC or IsVanilla,
	power_restoration_full = IsVanilla or isBCC,
	extra_power0 = true,
	...
	ShiftFocus = isCata or isWLK or isBCC,
...
if IsVanilla then		-- get_mana_regen_tick_from_gear
	local mp5_gear = {
		[9448] = 3,
		[10659] = 5,
		[11634] = 3,
...
function UF.create_power_restoration(U, unit)	-- TODO timer for different power type		-- DONE
	-- if true then return; end
	if not (IsVanilla or isBCC) then
		return;
	end
	...
	if U.CLASS == "DRUID" then
		...
		UF.regUnitEvent(U, unit, "UNIT_MAXPOWER", "UNIT_POWER_FREQUENT", "UNIT_SPELLCAST_SUCCEEDED");
...
function UF.createPartyAura(U, unit)
	...
	function U:UpdateAura()
		...
		if n_shown_debuffs > 0 then
			...
			if isCata or isWLK or isBCC then
...
local function init()
	do	--	player & pet
		...
		if isWLK or isBCC or IsVanilla then
	...
	--	setting
		...
		if (isCata or isWLK or isBCC) and alaUnitFrameSV.ShiftFocus then
...
do
	...
	function UF.ADDON_LOADED(self, event, addon)
		...
		if alaUnitFrameSV._ver == nil or alaUnitFrameSV._ver < 20241201.0 and IsVanilla then
		...
		alaUnitFrameSV._ver = 20241201.0;
